...
export const NotificationSection = (): JSX.Element => {
  const { items, loading } = useIngredientStore();

  const lowStockItems = useMemo(
    () => items.filter(i => i.stockStatus === "low" || i.stockStatus === "out"),
    [items]
  );
      ...
      <div className="bg-amber-100 border border-amber-200 p-6 rounded-md">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            {loading ? (
              <div className="text-sm text-amber-700">Đang tải dữ liệu tồn kho...</div>
            ) : lowStockItems.length === 0 ? (
              <div className="text-sm text-amber-700">Hiện không có nguyên liệu sắp hết.</div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {lowStockItems.slice(0, 6).map((ingredient) => (
                  <Card key={ingredient.id} className="bg-white border-amber-200">
                    <CardContent className="p-4">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <span className="[font-family:'Inter-Bold',Helvetica] font-bold text-red-600 text-base">
                          {typeof ingredient.stock === 'number' ? ingredient.stock : '—'} {ingredient.unit ?? ''}
                        </span>
                      </div>
