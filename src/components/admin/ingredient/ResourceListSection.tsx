...
import { Grid3X3, List, Minus, Plus, MoreVertical, Edit3, Trash2 } from "lucide-react";
    ...
    <section className="w-full bg-white rounded-xl border border-solid border-gray-200 shadow-[0px_1px_3px_#0000001a] overflow-hidden">
      ...
      <div className="p-6">
        <div className={viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6" : "grid grid-cols-1 gap-3"}>
          {filteredItems().map((ingredient: { id: number; name: string; unit?: string; category?: unknown; stock?: number; stockStatus?: 'out'|'low'|'normal' }) => (
            <Card key={ingredient.id} className={`bg-white border-2 border-gray-200`}>
              <CardContent className="p-6 space-y-4">
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3">
                    <div className={`w-3 h-3 ${ingredient.stockStatus === 'out' ? 'bg-red-700' : ingredient.stockStatus === 'low' ? 'bg-amber-700' : 'bg-emerald-700'} rounded-md opacity-60 mt-2`} />
                    <div>
                  ...
                  <div className="relative">
                    ...
                    <div className="absolute right-0 mt-1 bg-white border rounded shadow hidden z-10">
                      ...
                      <button className="px-3 py-2 flex items-center gap-2 w-full hover:bg-gray-100" onClick={()=>{
                        if (confirm('Xóa nguyên liệu này?')) remove(ingredient.id)
                      }}>
                ...
                <div className="flex gap-2">
                  <Button variant="secondary" size="icon" className={`w-8 h-8 bg-[#0000001a] hover:bg-[#0000002a]`}>
                  ...
                  <Button variant="secondary" className="h-auto bg-[#00000033] hover:bg-[#00000040] px-3 py-1 ml-auto">
                    <span className={`[font-family:'Arial-Narrow',Helvetica] font-normal text-gray-700 text-xs text-center`}>Cập nhật tồn kho</span>
                  </Button>
                </div>
